...
impl<T: Parsed> ParsedWithString for MemberAttribute<T> {
    fn parse(reader: &mut impl Read, constant_pool: &[cp_info]) -> std::io::Result<Self> {
        ...
        match name.as_ref() {
            "Synthetic" => check_attr_length(name, 0, attribute_length, Self::Synthetic),
            "Signature" => check_attr_length(
                name,
                2,
                attribute_length,
                Self::Signature {
            ...
            "Deprecated" => check_attr_length(name, 0, attribute_length, Self::Deprecated),
...
fn check_attr_length<T>(attr: &str, expected: u4, actual: u4, output: T) -> std::io::Result<T> {
    if actual != expected {
        Err(Error::new(
            ErrorKind::InvalidData,
            format!("{attr} attribute must have attribute_length {expected}; found {actual}"),
        ))
    } else {
        Ok(output)
    }
...
impl ParsedWithString for ClassAttribute {
    fn parse(reader: &mut impl Read, constant_pool: &[cp_info]) -> std::io::Result<Self> {
        ...
        match name.as_ref() {
            ...
            "EnclosingMethod" => check_attr_length(
                name,
                4,
                attribute_length,
            ...
            "ModuleMainClass" => check_attr_length(
                name,
                2,
                attribute_length,
                Self::ModuleMainClass(usize::parse(reader)? - 1),
            ),
            "NestHost" => check_attr_length(
                name,
                2,
                attribute_length,
                Self::NestHost(usize::parse(reader)? - 1),
            ),
...
impl ParsedWithString for FieldAttribute {
    fn parse(reader: &mut impl Read, constant_pool: &[cp_info]) -> std::io::Result<Self> {
        ...
        match name.as_ref() {
            "ConstantValue" => check_attr_length(
                name,
                2,
                attribute_length,
                Self::ConstantValue {
